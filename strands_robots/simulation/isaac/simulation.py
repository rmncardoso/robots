...
class IsaacSimulation(IsaacMotionPrimitivesMixin, IsaacRandomizationMixin, IsaacRecordingMixin, SimEngine):
    ...
    **kwargs
    ...
    def run_policy(
        ...
        Every parameter is spelled out and forwarded rather than absorbed by a
        ``**kwargs`` sink, which is a correctness requirement and not a style
        choice: a sink accepts EVERY keyword, so ``run_policy(instrction="pick")``
        would bind silently instead of raising ``TypeError``, and the typo would
        surface as a rollout that ignored the instruction. The signature is also
        what the documentation checks read to decide which keywords an example may
        pass - a sink reads to them as "accepts anything", which silently disables
        that check for every documented call of this method, not just this
        backend's. The MuJoCo backend's override spells them out for the same
        reason.
        ...
        Args:
            ...
            policy_provider: forwarded unchanged to :meth:`SimEngine.run_policy`.
            policy_config: forwarded unchanged.
            instruction: forwarded unchanged.
            duration: forwarded unchanged.
            control_frequency: forwarded unchanged.
            action_horizon: forwarded unchanged.
            fast_mode: forwarded unchanged.
            video: forwarded unchanged.
            policy_object: forwarded unchanged.
            n_steps: forwarded unchanged.
            max_steps: forwarded unchanged.
            max_onframe_failures: forwarded unchanged.
            control_substeps: forwarded unchanged.
            policy_kwargs: forwarded unchanged.
            seed: forwarded unchanged.
            n_episodes: forwarded unchanged.
            reset_between: forwarded unchanged.
            async_rtc: forwarded unchanged.
