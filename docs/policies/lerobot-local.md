...
(`embodiment.UNIT_FRAMES`) and any other is refused when the map is built:
`"DEGREES"`, the spelling LeRobot's own `MotorNormMode` uses, would otherwise
mean `"native"` and convert nothing.
